...
3. **配置隔离**：每个模块用独立前缀的环境变量（如 `LLM_*` / `RAG_*` / `DATABASE_URL`），避免冲突

### 1.5 开发建议

- 构建顺序：环境搭建（步骤0）→ infrastructure → tools / llm / utils → domain → apps（对应步骤1 → 步骤2-5 → 步骤6 → 步骤7）
- 功能最小可用即可，不做过度设计
- 增量式开发，每完成一个步骤就验证
...
在编写代码之前，先搭建统一的数据库环境。本项目使用 **PostgreSQL + pgvector** 扩展，将关系数据和向量数据统一存储在同一数据库中——这是当前企业级开发的主流选型。

### 0.1 为什么用 PostgreSQL 统一存储

| 对比 | SQLite + ChromaDB（原方案） | PostgreSQL + pgvector（新方案） |
|------|---------------------------|-------------------------------|
| 存储 | 两个独立存储，无事务保证 | 单库事务，关系数据与向量数据原子写入 |
| 并发 | SQLite 写锁瓶颈 | MVCC，高并发读写 |
| 运维 | ChromaDB 自管，SQLite 文件级 | pgvector 是 PG 扩展，运维生态极其成熟 |
| 向量检索 | ChromaDB 内置 HNSW | pgvector 支持 IVFFlat / HNSW |
| 部署 | 无需外部服务 | Docker Compose 一行命令启动 |

由于后续扩展为电商平台 + 智能客服双服务时，两个服务使用同一个 PostgreSQL 实例但**不同的 schema** 隔离数据，现在搭建的环境可直接复用。

### 0.2 Docker Compose 启动 PostgreSQL
...
创建 `init.sql`，Docker Compose 启动时自动执行。此文件是**唯一的建表入口**，后续步骤不再各自建表：
...
更新 `requirements.txt`，新增 PostgreSQL 驱动和 pgvector 客户端，移除 ChromaDB：
...
> `psycopg2-binary` 是 PostgreSQL 的 Python 驱动，`pgvector` 是 pgvector 的 Python 客户端。现有 `openai`、`fastapi`、`uvicorn`、`python-dotenv` 等依赖保持不变。

### 0.5 环境变量

`.env` 中新增数据库连接串，删除旧的 ChromaDB 和 SQLite 路径配置：

```bash
# ---- 数据库 ----
DATABASE_URL=postgresql://user:1234@localhost:5432/agent
```
...
# 3. 连接验证 schema 存在
docker compose exec postgres psql -U user -d agent -c "\dn"
# 应显示 customer_service 和 shop 两个 schema

# 4. 确认 pgvector 扩展可用
docker compose exec postgres psql -U user -d agent -c "SELECT extname FROM pg_extension;"
# 应显示 vector

# 5. 确认所有业务表已创建
docker compose exec postgres psql -U user -d agent -c "\dt customer_service.*"
# 应显示 faq_embeddings, conversations, messages 三张表
...
- 两个 schema（`customer_service`、`shop`）已创建
- pgvector 扩展已启用
- `customer_service.faq_embeddings`、`conversations`、`messages` 三张表已创建
- `DATABASE_URL` 已在 `.env` 中配置
- Python 依赖安装完成
...
| `rag/` 整个目录 | `infrastructure/rag/` | **移动**（内部代码需调整：ChromaDB → pgvector） |
...
> **注意**：`.env` 中 `DATABASE_URL` 已在步骤0 配置完成。`LLM_*` 和 `RAG_EMBEDDING_MODEL` 环境变量保持不变。项目提供了 `.env.example` 作为环境变量模板（可提交 git），请将其复制为 `.env` 并填入你自己的 API Key。`.env` 包含密钥，**不应提交到 git**。
...
4. **`.env` 文件存在**且 `DATABASE_URL`、`LLM_API_KEY`、`LLM_BASE_URL`、`LLM_MODEL` 均已配置
...
函数职责：
1. 从 `infrastructure.rag` 导入模块单例 `vector_store`，调用 `vector_store.search(query, top_k=top_k)`
...
> **已变更**：存储后端从 ChromaDB 改为 PostgreSQL + pgvector。如果你已完成 guide.md 的第三章（ChromaDB 版本），本章需要**重新实现** `store.py`，但 `embedding.py`（嵌入模型调用）可复用。

### 3.1 设计概览

FAQ 向量存储在 PostgreSQL 的 `customer_service.faq_embeddings` 表中，与步骤4 的会话数据共享同一个数据库连接池。

```
faq_embeddings 表（customer_service schema）
├── id          SERIAL PRIMARY KEY
├── question    TEXT               ← FAQ 问题原文
├── answer      TEXT               ← FAQ 答案原文
├── embedding   vector(1536)       ← OpenAI text-embedding-3-small 生成的向量
└── metadata    JSONB              ← 预留扩展字段
```

检索方式：`SELECT ... ORDER BY embedding <=> query_vector LIMIT k`（`<=>` 是 pgvector 的余弦距离运算符）。

### 3.2 确认表已就绪

`customer_service.faq_embeddings` 表及 HNSW 索引已在步骤0 的 `init.sql` 中创建（Docker Compose 启动时自动执行）。执行以下命令确认表存在且结构正确：

```bash
docker compose exec postgres psql -U user -d agent -c "\d customer_service.faq_embeddings"
```

应看到 `id`、`question`、`answer`、`embedding (vector(1536))`、`metadata (jsonb)` 五个字段，以及 `idx_faq_embedding` 索引。

> **无需手动建表**。如果表不存在，说明 Docker Compose 启动时 `init.sql` 未正确执行——检查 `docker compose down -v && docker compose up -d` 重新初始化。

### 3.3 VectorStore 实现

`infrastructure/rag/store.py` 需要重写。核心变化：

| 维度 | ChromaDB（旧） | pgvector（新） |
|------|-------------|-------------|
| 连接 | `chromadb.PersistentClient(path)` | 从 `infrastructure.database` 的池中获取连接 |
| 写入 | `collection.add(embeddings=..., documents=..., metadatas=...)` | `INSERT INTO customer_service.faq_embeddings (question, answer, embedding) VALUES (...)` |
| 检索 | `collection.query(query_embeddings=..., n_results=k)` | `SELECT ... FROM customer_service.faq_embeddings ORDER BY embedding <=> %s::vector LIMIT k` |
| 初始化 | ChromaDB 自动管理 schema | 建表已在 `init.sql` 完成，`VectorStore` 不负责建表 |

**对外接口不变**——`search(query_text, top_k)` 签名和返回格式保持一致，上层 Tool 层无需任何修改。

```python
# infrastructure/rag/store.py（核心逻辑示意）

class VectorStore:
    def __init__(self, pool):
        """pool: psycopg2 连接池，从 infrastructure.database 获取"""
        self.pool = pool

    def search(self, query_text: str, top_k: int = 5) -> list[dict]:
        # 1. 将 query_text 转为向量（复用 embedding.py）
        ...
        # 3. 格式化为统一结构
        return [
            {"id": f"faq_{r[0]}", "content": f"问题: {r[1]}\n答案: {r[2]}",
             "metadata": {"question": r[1], "answer": r[2]}, "distance": float(r[3])}
            for r in rows
        ]
```

> **embedding() 函数可复用**：原 guide.md 中 `rag/embedding.py` 的嵌入模型调用逻辑不变——从环境变量读取模型名和 API Key，调用 OpenAI Embedding API 返回向量。

### 3.4 FAQ 数据导入

ChromaDB 中的旧向量数据无法直接迁移到 pgvector（序列化格式不同），需要从原始 FAQ JSON 文件**重新导入**。更新 `scripts/import_faq.py`，核心变化：

```
旧流程（ChromaDB）：
  FAQ JSON → embed → collection.add(embeddings, documents, metadatas)

新流程（pgvector）：
  FAQ JSON → embed → INSERT INTO customer_service.faq_embeddings
```

导入脚本需完成以下步骤：

1. 读取 `data/faq.json`（或原 FAQ 数据文件），解析为 `[{question, answer}, ...]` 列表
2. 遍历每条，调用 `embed(question)` 生成向量（复用 `infrastructure/rag/embedding.py`）
3. 使用 `psycopg2` 连接到 PostgreSQL，执行参数化 INSERT
4. 打印进度（如 `已导入 15/50 条`）
5. 导入完成后打印总数，便于验证

> **容易踩的坑**：`embedding` 列类型是 `vector(1536)`，INSERT 时需用 `%s::vector` 做类型转换，或者使用 `pgvector` 包的 `Vector` 类型包装：`from pgvector.psycopg2 import Vector` → `Vector(embedding_list)`。

### 3.5 导入后验证 — 确认 RAG 检索可用

导入完成后，**不要直接进入下一步**，先验证向量检索功能正常：

```bash
# 1. 确认写入条数
docker compose exec postgres psql -U user -d agent \
  -c "SELECT COUNT(*) FROM customer_service.faq_embeddings;"
# 应与 FAQ JSON 中的条目数一致

# 2. 用 Python 验证检索（项目根目录下执行）
python -c "
from infrastructure.database import pool
from infrastructure.rag import vector_store
results = vector_store.search('退货流程', top_k=3)
for r in results:
    print(f\"[{r['id']}] distance={r['distance']:.4f} | {r['metadata']['question']}\")
"
# 应返回 3 条与"退货流程"相关的结果，distance 值越小越相似
```

> 如果检索返回 0 条或报错，检查：① `import_faq.py` 是否已执行且成功 ② `embedding` 列的类型是否为 `vector(1536)` ③ 嵌入 API（OpenAI）是否正常响应。
...
vector_store = VectorStore(pool)  # 模块单例，步骤2 即可 import 使用
```

> `vector_store` 是模块级单例，在 `__init__.py` 加载时即创建。`pool` 来自 `infrastructure/database.py` 的模块级连接池（通过 `DATABASE_URL` 环境变量初始化）。

### 3.7 完成标志

- `faq_embeddings` 表已在步骤0 的 `init.sql` 中创建，HNSW 索引就绪
- `import_faq.py` 执行成功，写入条数与 FAQ JSON 中的条目数一致（用 `SELECT COUNT(*)` 确认）
- `vector_store.search("退货流程")` 返回相关结果，`distance` 值越小表示越相似
- 返回格式与 3.3 节定义一致（`id` / `content` / `metadata` / `distance`）
- 3.5 节的两条验证命令均正常输出
...
**已有条件**：
- `infrastructure/rag/embedding.py` 可复用，提供 `embed(text) -> list[float]` 函数
- 数据库连接池由 `infrastructure/database.py` 提供，`pool.getconn()` / `pool.putconn()` 获取/归还连接
- `customer_service.faq_embeddings` 表已在 `init.sql` 中创建

**需要创建/修改的内容**：
- `infrastructure/rag/store.py`：`VectorStore` 类，`__init__(self, pool)`，`search(query_text, top_k) -> list[dict]`
- `scripts/import_faq.py`：从 ChromaDB API 改为 PostgreSQL INSERT（读取 FAQ JSON → embed → INSERT）

**关键约束**：
- `search()` 返回格式必须保持：`[{id, content, metadata: {question, answer}, distance}, ...]`
- 向量检索 SQL：`ORDER BY embedding <=> %s::vector LIMIT k`
- `embedding` 列类型为 `vector(1536)`（text-embedding-3-small 维数），INSERT 时用 `pgvector.psycopg2.Vector` 包装嵌入列表，检索时传入的 query_vector 用 `%s::vector` 做类型转换
- `VectorStore` 不负责建表——表已在 `init.sql` 中创建
- 导入脚本执行后，用 `SELECT COUNT(*) FROM customer_service.faq_embeddings` 确认条数，用 `vector_store.search("测试")` 验证检索可用

### 3.9 常见偏差

- 忘记在 SQL 中做 `%s::vector` 类型转换导致 pgvector 报类型不匹配
- 返回格式中 `distance` 字段名写错（不是 `score`）
- 在 `VectorStore.__init__()` 中建表（建表应在 `init.sql` 中完成）
...
行级存储，两张表。**DDL 已在步骤0 的 `init.sql` 中执行完毕**，此处列出表结构供理解：

| 表 | 关键列 | 说明 |
|----|--------|------|
| `customer_service.conversations` | `id TEXT PK`, `user_id`, `title`, `status`, `created_at`, `updated_at` | 会话元数据 |
| `customer_service.messages` | `id SERIAL PK`, `conversation_id FK`, `role`, `content`, `turn_number`, `metadata JSONB`, `created_at` | 消息明细，索引 `(conversation_id, turn_number)` |

> **PostgreSQL 与 SQLite 差异**：`AUTOINCREMENT` → `SERIAL`；`datetime('now')` → `NOW()`；`metadata` 从 `TEXT` 升级为 `JSONB`（原生 JSON 类型，psycopg2 自动序列化/反序列化，无需手动 `json.dumps`/`json.loads`）。`ConversationManager` 不负责建表。
...
add_message(conversation_id, role, content):
  ...
  查询该会话当前的 max(turn_number)：
    SELECT COALESCE(MAX(turn_number), 0) FROM customer_service.messages WHERE conversation_id = %s
...
class ConversationManager:
    def __init__(self, pool, max_context_turns: int = 10):
        """
        pool: psycopg2 连接池，从 infrastructure.database 获取
...
- 使用 `psycopg2`（`psycopg2-binary` 包）连接 PostgreSQL，通过 `pool.getconn()` / `pool.putconn()` 获取和归还连接。
- `__init__()` 中**不建表**——建表已在 `init.sql` 中完成，由 Docker Compose 启动时自动执行。`ConversationManager` 只负责 CRUD。
- 所有 SQL 使用 `%s` 占位符（psycopg2 风格，不是 SQLite 的 `?`）。
- 每个 CRUD 方法从池中取连接、执行 SQL、提交、归还连接（不关闭连接，由池管理生命周期）。
- 表名前缀 `customer_service.`（或在连接时 `SET search_path TO customer_service`）。
- `metadata` 列类型为 `JSONB`（不是 TEXT），Python dict 可直接作为参数传入，psycopg2 自动序列化。读取时自动反序列化为 dict。
...
- `conversations` 和 `messages` 两张表已在 PostgreSQL 中创建（在 `init.sql` 中定义）
...
**已有条件**：
- PostgreSQL + pgvector 已在步骤0 启动，`DATABASE_URL` 在 `.env` 中配置
- `customer_service.conversations` 和 `customer_service.messages` 两张表已在 `init.sql` 中定义
- 数据库连接池由 `infrastructure/database.py` 提供（`pool.getconn()` / `pool.putconn()`）
- 依赖包 `psycopg2-binary` 已安装

**需要创建的内容**：
- `infrastructure/database.py`：创建 `psycopg2.pool.SimpleConnectionPool`，从 `DATABASE_URL` 读取连接串，对外暴露 `pool` 变量
...
**关键约束**：
- `ConversationManager.__init__(self, pool, max_context_turns=10)` 接收连接池，**不建表**（建表在 `init.sql` 中完成）
- 所有 SQL 使用 `%s` 占位符（psycopg2 风格），表名带 `customer_service.` schema 前缀
- `add_message()` 的 turn_number 计算：`SELECT COALESCE(MAX(turn_number), 0) FROM customer_service.messages WHERE conversation_id = %s`，role 为 `"user"` 时 +1，role 为 `"assistant"` 时复用当前值
- `get_context()` 截取最近 N 轮：`SELECT role, content FROM customer_service.messages WHERE conversation_id = %s ORDER BY turn_number DESC LIMIT %s`（LIMIT 值为 `max_context_turns * 2`），结果按 `turn_number ASC` 排序返回
- `get_history()` 无 LIMIT，其余同 `get_context()`
- `metadata` 列类型为 `JSONB`，Python dict 可直接传入，psycopg2 自动序列化/反序列化
- 每个方法从池取连接、执行、提交、归还（不关闭连接，由池管理生命周期）
...
| `from utils import ConversationManager` | `cm = ConversationManager(pool, max_context_turns=10)`；`cm.add_message()` / `cm.get_context()` / `cm.create()` | 会话持久化与上下文获取 |
...
```
依赖装配顺序（在 apps/customer_service/main.py 的启动事件或模块级完成）：

1. 加载 .env（dotenv）
2. 导入模块级单例（无需 new，直接 import 即用）
   → from llm import chat_service                 # LLM 客户端
   → from infrastructure.database import pool     # 数据库连接池
   → from infrastructure.rag import vector_store  # 向量存储
3. 创建需构造函数注入的实例
   → cm = ConversationManager(pool, max_context_turns=10)  # 注入连接池
...
> **模块单例 vs 构造函数注入**：`chat_service`、`pool`、`vector_store` 是固定依赖，用模块单例直接 import。`ConversationManager`、`CustomerServiceAgent` 有可变参数（pool、tools 列表），在 `main.py` 中用构造函数注入。
...
**已有模块接口**：
- `from infrastructure.database import pool` → `psycopg2.pool.SimpleConnectionPool`（模块单例，直接 import）
- `from infrastructure.rag import vector_store` → `VectorStore` 的模块单例，`.search(query, top_k)` 直接可用
- `ConversationManager(pool, max_context_turns).create()` / `add_message()` / `get_context()` / `get_history()` / `list_conversations()`
- `agent.run(user_input: str, conversation_id: str)` → `AgentResponse(type, content, conversation_id, metadata)`

**需要创建的内容**：
- `apps/customer_service/main.py`：FastAPI 应用入口，完成依赖装配（导入模块单例 + 创建 ConversationManager + Agent）
...
**关键约束**：
- `pool` 和 `vector_store` 是模块单例，直接 `from infrastructure.xxx import pool, vector_store`，无需在 main.py 中 new
- `ConversationManager` 注入 pool（`ConversationManager(pool, max_context_turns=10)`）
...
└──┬───────┬───────┘
   ↓       ↓
┌──────┐ ┌──────┐ ┌──────┐ ┌──────────────────┐
│ tools│ │ llm  │ │ utils│ │ infrastructure   │  ← 第 1、2 个构建
│      │ │      │ │      │ │  rag/            │    (互不依赖)
│      │ │      │ │      │ │  database        │
│      │ │      │ │      │ │  models          │
└──────┘ └──────┘ └──────┘ └──────────────────┘
...
1. `infrastructure/` — 数据持久化：rag（已完成）+ database + models
2. `tools/` + `llm/` + `utils/` — 原子能力：工具基类 + FAQ 检索 + LLM 客户端 + 会话管理（顺序不限，互不依赖）
3. `domain/customer_service/` — Agent（依赖 tools + llm + utils + infrastructure）
4. `apps/customer_service/` — API（依赖 domain）

### B. 数据流图（一次完整交互）

```
客户端                     apps/api                domain/agent              infrastructure          llm
  │                          │                        │                         │                    │
  │ POST /chat               │                        │                         │                    │
  │ {conv_id, "我要退"}       │                        │                         │                    │
  │─────────────────────────→│                        │                         │                    │
...
│   └── customer_service/
│       ├── __init__.py
│       └── routes.py                  # [调整] POST /chat, /conversations
│
├── domain/
│   └── customer_service/
│       ├── __init__.py                # 导出 CustomerServiceAgent
│       ├── agent.py                   # [新增] ReAct Agent + AskUser
│       └── prompts.py                 # [新增] 客服 prompt 模板
│
├── tools/
│   ├── __init__.py                    # 导出 Tool, ToolParameter, search_faq, search_faq_tool
│   ├── base.py                        # [新增] Tool + ToolParameter
│   └── faq_search.py                  # [新增] search_faq Tool
│
├── llm/
│   ├── __init__.py                    # 导出 LLMClient
│   ├── client.py                      # [移动] 从原 model/ 移入，不变
│   └── exceptions.py                  # [移动] 从原 model/ 移入，不变
│
├── utils/
│   ├── __init__.py                    # 导出 ConversationManager
│   └── conversation.py                # [新增] 会话存储 + 上下文构建
│
├── infrastructure/
│   ├── rag/
│   │   ├── __init__.py                # 导出 VectorStore（不变）
│   │   ├── embedding.py               # 嵌入模型（不变）
│   │   ├── store.py                   # 向量检索（不变）
│   │   └── faq_loader.py              # FAQ 数据加载（不变）
│   ├── database.py                    # [新增] 数据库连接管理
│   └── models.py                      # [新增] Message, Conversation ORM
│
├── data/                              # 本地数据（不提交 git）
│   └── faq.json                        # FAQ 数据文件
│
├── scripts/                           # 辅助脚本（更新 import 路径，按需保留）
│
├── docker-compose.yml                  # PostgreSQL + pgvector 容器
├── init.sql                           # Schema 初始化 + 建表
...
| 决策 | 结论 | 理由 |
|------|------|------|
| 工具注册 | Agent `self.tools` 列表，无独立 Registry | 减少抽象层，新增工具追加列表即可 |
| 工具注册 | 函数 + Tool 对象分离注册 | 函数供后端 import，Tool 对象供 Agent 调度，互不污染 |
| OpenAI function calling | 预留 `to_openai_schema()`，不实现 | 当前正则解析足够，未来可平滑迁移 |
| 会话存储 | PostgreSQL 行级存储（JSONB 原生支持） | 与向量数据同库，事务一致，窗口截取简单 |
| 多轮关系 | `(conversation_id, turn_number)` | 排序和截取只需一个复合索引 |
| 上下文截取 | 按轮数（默认 10 轮），SQL 层截取 | 客服对话短，轮数控制足够 |
| 上下文注入 | `{context}` 占位符 → SQL 历史 → 格式化文本 → 替换 | 简单直接，LLM 易于理解 |
| 检索策略 | 单一语义检索 | FAQ 场景不需要多策略 |
| 查询改写 | 去掉，由 Agent Thought 替代 | Agent 自带推理能力，改写冗余 |
| 数据存储 | PostgreSQL + pgvector 统一存储 | 关系数据与向量数据在同一库中，事务一致、运维简单 |
| Schema 隔离 | `customer_service` vs `shop` | 两个服务的表分属不同 schema，数据所有权清晰，为后续拆分服务预留边界 |
| AskUser | 一等 Action，不是 Tool | 反问需要中断循环，Tool 执行模式做不到 |
| Thought/Observation | 内存 `step_history` 列表，不入 SQL | 它们是 Agent 工作状态，不是对话 transcript |
| 记忆系统 | 暂不引入 | 会话管理已覆盖当前需求 |
| 用户会话隔离 | 暂不实现 | 当前阶段 `conversation_id` 即可访问任意会话。生产环境需在 API 层增加身份认证（如 JWT），校验请求者是否持有该会话 |

> **已知限制**：当前版本未做会话的用户权限校验，任意持有 `conversation_id` 的人均可访问对应会话。这是教学阶段的故意简化，后续扩展为电商智能客服时必须补齐用户认证与会话隔离。

### E. 扩展：反问能力

完成步骤6 的基础 ReAct 循环（search_faq + Finish）后，叠加第三种 Action：**向用户反问（AskUser）**。

> **本扩展是增量修改，不会推倒重来。** 只需在三个位置追加代码，基础的 search_faq + Finish 循环保持不变。

#### E.1 为什么 AskUser 是 Action 而非 Tool

- `search_faq` → 同步执行，立即返回 Observation → 循环继续
- `AskUser` → 需要外部交互 → 循环暂停 → 返回反问内容给用户 → 下一轮用户回复后重新进入

如果将反问注册为 Tool，Agent 循环不知道何时暂停等待用户输入。必须作为一等 Action，由 `run()` 识别并中断循环。

#### E.2 需要修改的位置

| 位置 | 修改内容 |
|------|---------|
| `AgentResponse.type` | 增加可选值 `"ask_user"` |
| Prompt 模板 | 在 Action 列表中追加 `AskUser[反问内容]` 动作说明 + 触发规则（给出具体场景示例，如"用户只说'退'但未明确退货还是退款"） |
| `run()` 3c 步骤 | 在判断 `Finish` **之前**增加 AskUser 判断分支：`action.startswith("AskUser")` → 提取反问内容 → 持久化 assistant 消息 → `return AgentResponse("ask_user", ...)` |
| `_parse_action_input()` | 增加 `AskUser[反问]` → `"反问"` 的提取逻辑 |

#### E.3 反问流程示例

```
第1轮请求:
  POST /chat  {"conversation_id":"c001", "message":"我要退"}
  → Agent.run("我要退", "c001")
  → get_context() 返回空（首轮对话）
  → LLM 输出: Thought: 用户意图模糊→ Action: AskUser[请问您是想退货还是退款？]
  → messages 表写入 assistant="请问您是想退货还是退款？"
  → 返回: {"type":"ask_user", "content":"请问您是想退货还是退款？"}

第2轮请求:
  POST /chat  {"conversation_id":"c001", "message":"退货"}
  → Agent.run("退货", "c001")
  → get_context() 返回:
     用户: 我要退
     客服: 请问您是想退货还是退款？
  → LLM 输出: Thought: 用户要退货→ Action: search_faq[退货流程]
  → Observation: [FAQ-003] 退货需在7天内申请，商品完好...
  → LLM 输出: Thought: 信息充足→ Action: Finish[退货流程如下：1. ...]
  → messages 表写入 assistant="退货流程如下：1. ..."
  → 返回: {"type":"final_answer", "content":"退货流程如下：1. ..."}
```

#### E.4 完成标志

- 意图模糊时触发 AskUser 反问，意图清晰时检索并回答
- 一次完整的"反问 → 用户回复 → 最终答案"多轮交互跑通

#### E.5 AI 协作提示

将以下信息提供给 LLM：

**已有条件**：步骤6 的基础 ReAct Agent 已跑通，`run()` 循环中只有 `Finish` 判断分支，`AgentResponse.type` 只有 `"final_answer"`。

**修改内容**（均为增量追加，不删除已有逻辑）：
1. `AgentResponse` 的 `type` 字段增加 `"ask_user"` 可选值
2. Prompt 模板的 Action 列表中追加 `AskUser[反问内容]` — 在意图模糊时向用户提问澄清
3. `run()` 的 3c 步骤中，在 `Finish` 判断**之前**增加 `AskUser` 判断：`if action.startswith("AskUser")` → 提取反问内容 → `cm.add_message(...)` 持久化 → `return AgentResponse("ask_user", ...)`
4. `_parse_action_input()` 中增加对 `AskUser[反问]` 的解析

**关键约束**：
- AskUser 的判断必须在 Finish 判断**之前**（否则可能被误判为工具调用）
- 持久化 assistant 消息时，可附带 metadata `{"action_type": "ask_user"}` 便于后续分析
- 基础的 search_faq + Finish 逻辑完全不变

### F. 架构演进路径（概览）

当前设计为单体应用。后续扩展为"电商平台 + 智能客服"双服务时，应遵循以下演进方向，**无需在当前阶段预留代码**：

| 阶段 | 内容 | 关键变化 |
|------|------|---------|
| **当前** | 单体 FastAPI + PostgreSQL + pgvector | 验证 Agent 逻辑，`customer_service` schema 下建表 |
| **Phase 2** | 拆为两个 FastAPI 服务 | `shop` schema 承载电商数据，增加 JWT 认证，跨服务通过 API 调用（不直连对方数据库） |
| **Phase 3** | 引入 API 网关 | 统一路由、认证、限流（nginx/kong） |
| **Phase 4** | 异步任务 + 可观测性 | 消息队列处理异步操作，Prometheus + Grafana 监控 |

当前架构中的以下设计可平滑承接到多服务阶段：
- `tools/` 目录可按业务域分包（`tools/faq/`、`tools/order/`），新增远程调用 Tool 不改变框架
- `domain/` 可按业务域拆分（`domain/customer_service/`、`domain/shop/`），各域独立演进
- 模块单例模式适用于单服务内的固定依赖；跨服务依赖通过 API 调用或消息队列解耦
- **Schema 隔离设计**：当前 `customer_service` 和 `shop` 两个 schema 已在 PostgreSQL 中创建。拆分为双服务时，只需为每个服务配置独立的数据库用户并授权对应 schema，无需迁移数据或改表结构。两个服务各拥有一套连接池，互不干扰
