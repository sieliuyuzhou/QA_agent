...
> **本步骤需要新建/改写的文件**：`apps/customer_service/__init__.py`、`apps/customer_service/main.py`、`apps/customer_service/routes.py`。原有的 `api/` 目录在步骤1 迁移中已删除，本步骤在 `apps/customer_service/` 下重新实现 API。
...
> **模块单例 vs 构造函数注入**：`chat_service` 是固定依赖（全局一份配置），用模块单例直接 import。`ConversationManager` 和 `CustomerServiceAgent` 有可变参数（db_path、tools 列表），在 main.py 中用构造函数注入。
...
**需要创建的内容**：
- `apps/customer_service/main.py`：FastAPI 应用入口，完成依赖装配
...
- FastAPI app 的创建和路由注册在 `main.py` 中完成
...
| 会话存储 | SQLite 行级存储 | 窗口截取简单（`LIMIT N*2`），扩展性好 |
...
| 阶段 | 内容 | 关键变化 |
|------|------|---------|
| **当前** | 单体 FastAPI + SQLite | 验证 Agent 逻辑，单机运行 |
| **Phase 2** | 拆为两个 FastAPI 服务 | 共享 PostgreSQL，增加 JWT 认证 |
...
- `infrastructure/` 中的 SQLite 可替换为 PostgreSQL，ConversationManager 接口不变
