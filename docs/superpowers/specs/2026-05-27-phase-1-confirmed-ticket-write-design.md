...
4. 仅当 `eligible=True`、`recommended_service` 为允许工单类型，且与用户本次明确申请的 `request_type` 一致时创建动作。
5. 保存规则重算所需的输入和创建时的资格代码，不接收客户端提供的资格结论。

若规则仅给出替代路径建议，例如用户请求退换但规则结论为可选择付费维修，服务端不得自动生成付费维修工单动作；用户需明确选择新的办理类型后重新发起请求。
...
| 规则建议的办理类型与当前申请不同 | `409`，仅返回替代建议，不自动生成另一类型工单动作 |
| 动作非归属或不存在 | `404` |
| 动作过期或已取消 | `409`，不建单 |
